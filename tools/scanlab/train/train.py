...
def main():
    ...
    mlp = results["MLP (24, 12)"]["model"]
    # Six decimal places on the layer weights. They came out of a stochastic
    # fit on simulated data, so digits past that are noise being shipped to
    # every visitor as bytes, and they are all order-1 so six places is about
    # seven significant figures.
    #
    # The scaler is deliberately NOT rounded. Its terms are divided by, and the
    # smallest scale here is 0.049, where six decimal places leaves only five
    # significant figures — enough relative error to shift a prediction by 5e-5
    # once it compounds through the layers. That is 72 numbers against roughly
    # 1,200 weights, so full precision costs almost nothing and rounding them
    # buys almost nothing. An assertion below fails if this stops being true.
    def trim(a):
    ...
    # The model the app actually loads, written straight into src so nothing
    # is hand-copied between training and shipping.
    shipped = ROOT / "src" / "lib" / "scanModel.data.json"
    shipped.write_text(json.dumps(weights))
    kb = shipped.stat().st_size / 1024
    print(f"\n  Exported MLP weights: {shipped.relative_to(ROOT)} ({kb:.1f} KB)")
    print(
    ...
    # Parity fixture. The TypeScript forward pass has to reproduce these
    # numbers exactly, which is what turns "no train/serve skew" from an
    # intention into something a test fails on.
    #
    # The expected values are computed from the *rounded* weights that actually
    # ship, not from scikit-learn's full-precision model. Comparing against the
    # unrounded model would build a permanent ~1e-7 discrepancy into the test
    # and force a tolerance loose enough to hide a real bug. The rounding is
    # verified harmless separately, just below.
    def forward(vector):
        a = (np.asarray(vector, dtype=np.float64) - np.asarray(weights["mean"])) / np.asarray(
            weights["scale"]
        )
    ...
    sk = mlp.predict_proba(Xs_te)[:, 1]
    ours = np.array([forward(v) for v in X_te])
    drift = float(np.max(np.abs(sk - ours)))
    print(f"  Weight rounding shifts predictions by at most {drift:.2e}")
    # Rounding the layer weights halves the shipped file, from 27 KB to 14 KB.
    # The cost is a shift of order 1e-5 on the least typical inputs, where a
    # standardised feature several sigma out amplifies the per-weight error
    # through the layers. The UI reports three bands with edges at 0.2 and 0.8,
    # so nothing within three orders of magnitude of this can change what a
    # user sees. The bound is asserted rather than assumed so that a future
    # change to the rounding cannot quietly cross the line.
    assert drift < 1e-3, f"rounding moved predictions by {drift:.2e}"

    rng = np.random.default_rng(SEED)
    sample = rng.choice(len(X_te), size=24, replace=False)
    parity = {
        "note": (
            "Generated by tools/scanlab/train/train.py from the rounded weights "
            "in scanModel.data.json. The TypeScript forward pass must match "
            "these exactly."
        ),
        "cases": [
            {
                "features": [float(v) for v in X_te[i]],
                "expected": forward(X_te[i]),
            }
            for i in sample
        ],
    }
    fixture = ROOT / "src" / "lib" / "__tests__" / "scanModel.parity.json"
    fixture.write_text(json.dumps(parity, indent=2))
    print(f"  Parity fixture: {len(parity['cases'])} cases -> {fixture.relative_to(ROOT)}\n")
